...
export const ProvedorEstadoGlobal: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const editarFeedback = async (id: number, novoComentario: string) => {
    ...
    try {
      ...
      setFeedbacks(prevFeedbacks =>
        prevFeedbacks.map(feedback => (feedback.id === id ? updatedFeedback : feedback))
      );
    } catch (error) {
      console.error(error);
    }
  ...
  // Carrega os feedbacks do AsyncStorage ao iniciar
  useEffect(() => {
  ...
  // Salva os feedbacks no AsyncStorage sempre que eles forem atualizados
  useEffect(() => {
